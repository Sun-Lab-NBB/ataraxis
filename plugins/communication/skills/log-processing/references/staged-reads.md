...
- `breakdown` maps each filterable axis to its value counts, and it answers "which IDs, names, or statuses are present
  and how many of each" without listing anything. Read filter values from here.
...
- A filter naming a value the scan did not find returns an error dictionary naming what is available. An empty page
  therefore means the filters matched nothing that also survived paging, never a typo.
