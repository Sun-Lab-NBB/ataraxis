...
- `pool_size` follows the smallest of the job count, the core budget, and the spawned child baselines half the memory
  budget can hold, and never falls below one. That baseline is what a slot costs while it sits idle, and it is charged
  against the same half. Size a batch from `pool_size` and the budget together, never from the per-job `memory_mb`
